...
# ========== Output Directory ==========
experiment_name = "EXP_thermal_diffusion_with_k_and_f"
...
# ========== Data Generator ==========
def generate_single_sample(X, Y, h):
    m, n = np.random.randint(1, 6), np.random.randint(1, 6)
    Q = np.sin(m * np.pi * X) * np.sin(n * np.pi * Y)
    k = 0.5 + 0.5 * np.sin(2 * np.pi * X * np.random.uniform(1, 3)) * np.cos(2 * np.pi * Y * np.random.uniform(1, 3))
    T = solve_thermal_diffusion(k, Q, h)
    return k, Q, T

def generate_poisson_data(n_samples=1000, size=64):
    x = np.linspace(0, 1, size)
    y = np.linspace(0, 1, size)
    X, Y = np.meshgrid(x, y, indexing='ij')
    h = 1.0 / (size - 1)

    results = Parallel(n_jobs=-1)(delayed(generate_single_sample)(X, Y, h) for _ in range(n_samples))
    K_list, F_list, T_list = zip(*results)

    K = np.expand_dims(np.array(K_list), axis=-1)
    F = np.expand_dims(np.array(F_list), axis=-1)
    X_coords = np.broadcast_to(X[None, ..., None], K.shape)
    Y_coords = np.broadcast_to(Y[None, ..., None], K.shape)

    T = np.expand_dims(np.array(T_list), axis=-1)
    input_fields = np.concatenate([K, F, X_coords, Y_coords], axis=-1)

    input_fields = (input_fields - input_fields.mean()) / input_fields.std()
    T = (T - T.mean()) / T.std()
    return input_fields.astype(np.float32), T.astype(np.float32)
...
class FourierLayer(tf.keras.layers.Layer):
    def __init__(self, in_channels, out_channels, modes=12):
        ...
        self.modes = modes
        self.weights_real = self.add_weight(name="w_real", shape=[in_channels, out_channels, modes, modes], initializer=tf.keras.initializers.RandomNormal(stddev=1e-2), trainable=True)
        self.weights_imag = self.add_weight(name="w_imag", shape=[in_channels, out_channels, modes, modes], initializer=tf.keras.initializers.RandomNormal(stddev=1e-2), trainable=True)
    ...
    def call(self, x):
        ...
        out_full = tf.zeros([B, self.out_channels, H, W], dtype=tf.complex64)
        out_full = tf.concat([
...
class FNOBlock(tf.keras.layers.Layer):
    def __init__(self, width, modes=12):
        ...
        self.conv = tf.keras.layers.Conv2D(width, 1, padding='same', kernel_initializer='he_normal', kernel_regularizer=tf.keras.regularizers.l2(1e-4))
...
class FNO2D(tf.keras.Model):
    def __init__(self, modes=12, width=32):
        super().__init__()
        self.input_proj = tf.keras.layers.Conv2D(width, 1, padding='same', kernel_initializer='he_normal', kernel_regularizer=tf.keras.regularizers.l2(1e-4))
        self.blocks = [FNOBlock(width, modes) for _ in range(4)]
        self.output_proj = tf.keras.layers.Conv2D(1, 1, padding='same', kernel_initializer='he_normal', kernel_regularizer=tf.keras.regularizers.l2(1e-4))
...
# ========== Training ==========
inputs, targets = generate_poisson_data(n_samples=1000, size=64)
...
history = model.fit(train_x, train_y, validation_split=0.1, batch_size=32, epochs=100,
